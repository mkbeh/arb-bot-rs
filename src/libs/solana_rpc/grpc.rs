...
use crate::libs::solana_rpc::{
    Event,
    dex::{DEX_PARSERS, TxEvent},
};
...
impl GrpcClient {
        ...
        Callback: FnMut(Event) -> anyhow::Result<()> + Send + 'static,
    {
        ...
        self.callback = Some(EventCallback::new(callback));
        self
    }
    ...
    fn build_subscribe_request(
        ...
        let program_accounts = program_ids
            .iter()
        ...
        let transactions_filter = SubscribeRequestFilterTransactions {
            ...
            account_include: program_accounts,
            ..Default::default()
        };
                        ...
                        Ok(update) => {
                            if let Some(oneof) = update.update_oneof.as_ref()
                                && let Some(event) = parse_update_to_event(oneof) {
                                    // self.try_invoke_callback(event);
...
pub struct EventCallback {
    inner: Arc<Mutex<Box<dyn FnMut(Event) -> anyhow::Result<()> + Send + 'static>>>,
}
...
fn parse_update_to_event(event: &UpdateOneof) -> Option<Event> {
    match event {
        UpdateOneof::Transaction(tx) => {
            parse_tx_update_to_event(tx).map(|tx_event| Event::Tx(Box::new(tx_event)))
        }
        UpdateOneof::Slot(slot) => Some(Event::Slot {
            slot: slot.slot,
            status: slot.status,
            parent: slot.parent,
        }),
        UpdateOneof::BlockMeta(meta) => Some(Event::BlockMeta {
            slot: meta.slot,
...
fn parse_tx_update_to_event(tx: &SubscribeUpdateTransaction) -> Option<TxEvent> {
    let transaction_info = tx.transaction.as_ref()?;
    let confirmed_tx = transaction_info.transaction.as_ref()?;
    let message = confirmed_tx.message.as_ref()?;
    let instructions = &message.instructions;

    for instruction in instructions {
        let program_id_index = instruction.program_id_index as usize;
        let key = message.account_keys.get(program_id_index)?;

        let program_id = match Pubkey::try_from(key.as_slice()) {
            Ok(pubkey) => pubkey.to_string(),
            Err(_) => continue,
        };

        if let Some(parser) = DEX_PARSERS.get(&program_id) {
            let payload = match general_purpose::STANDARD.decode(&instruction.data) {
                Ok(bytes) => bytes,
                Err(_) => continue,
            };

            if let Some(tx_event) = parser(&payload) {
                return Some(tx_event);
            }
        }
    }
