...
pub async fn connect_ws(
    ...
    let ws_url = build_ws_url(conf.clone())?;
...
fn build_ws_url(conf: ConnectConfig) -> anyhow::Result<Url> {
    ...
    let original = format!("{}{}", conf.token, timestamp);
    ...
    let ws_url = format!(
        "{}/v1/private?{}&sign={}&passphrase={}",
        conf.ws_url, url_path, sign_value, passphrase_sign
    );
