...
pub struct OrderSymbol {
    ...
    pub order_book: Arc<BookTickerEvent>,
...
impl OrderBuilder {
    ...
    pub async fn build_chains_orders(
        ...
        for chain in chains.iter() {
            tasks_set.spawn({
                ...
                let this = self.clone();
                async move { this.build_orders(store, &base_assets, &chain).await }
            });
        }
    ...
    async fn build_orders(
        ...
        loop {
            // Get order book for each symbol in chain.
            let order_books = chain
                .clone()
                .into_iter()
                .filter_map(|s| store.get(s.symbol.symbol.as_str()))
                .collect::<Vec<_>>();

            if order_books.len() < chain.len() {
                tokio::time::sleep(Duration::from_secs(1)).await;
                continue;
            }

            let current_prices = chain
                .iter()
                .zip(order_books.iter())
                .map(|(symbol, ticker)| match symbol.order {
                    SymbolOrder::Asc => ticker.best_bid_price,
                    SymbolOrder::Desc => ticker.best_ask_price,
                })
                .collect::<Vec<_>>();
