...
1. Учитывать комиссии за сделки (нужно делать отдельно пересчет с учетом комиссиями???).
2. Учитывать погрешность по профиту (???).
3. Учитывать максимально допустимый объем средств на 1 сделку (задается в конфигурационном файле).
4. Добавить суммирование ордеров (если объем первого ордера в `order book`
   меньше объема из п.3, то попробовать суммировать объемы).
