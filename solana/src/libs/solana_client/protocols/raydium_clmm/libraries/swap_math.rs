...
use crate::libs::solana_client::protocols::raydium_clmm::{ErrorCode, FEE_RATE_DENOMINATOR_VALUE};
...
/// Therefore, this situation needs to be handled in `compute_swap_step` to recalculate the price
/// that can be reached based on the amount.
fn calculate_amount_in_range(
    ...
    match result {
        Ok(v) => Ok(Some(v)),
        Err(e) if e.downcast_ref::<ErrorCode>() == Some(&ErrorCode::MaxTokenOverflow) => Ok(None),
        Err(_) => Err(anyhow::anyhow!(ErrorCode::SqrtPriceLimitOverflow)),
    }
}
