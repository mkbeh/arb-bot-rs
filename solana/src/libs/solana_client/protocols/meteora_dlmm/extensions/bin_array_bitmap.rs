...
use crate::libs::solana_client::protocols::meteora_dlmm::{BinArrayBitmapExtension, constants::*};
...
impl BinArrayBitmapExtExtension for BinArrayBitmapExtension {
    ...
    fn iter_bitmap(&self, start_index: i32, end_index: i32) -> Result<Option<i32>> {
        if start_index == end_index {
            return if self.bit(start_index)? {
                Ok(Some(start_index))
            } else {
                Ok(None)
            };
        ...
        if start_index < 0 {
            ...
            if start_index < end_index {
                for i in (0..=offset).rev() {
                    ...
                    if i == offset {
                        ...
                        return Ok(Some(Self::compute_bin_array_index(
                            i,
            ...
            // iter in positive_bin_array_bitmap
            if start_index < end_index {
                for i in offset..EXTENSION_BINARRAY_BITMAP_SIZE {
