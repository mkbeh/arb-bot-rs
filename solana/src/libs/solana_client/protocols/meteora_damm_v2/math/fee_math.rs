...
pub fn get_fee_in_period(
    ...
    // Make bin_step into Q64x64, and divided by MAX_BASIS_POINT. If bin_step = 1, we get 0.0001 in
    // Q64x64
    let bps = u128::from(reduction_factor)
        .safe_shl(SCALE_OFFSET)?
        .safe_div(MAX_BASIS_POINT.into())?;
    let base = ONE_Q64.safe_sub(bps)?;
    let result = pow(base, passed_period.into()).ok_or(PoolError::MathOverflow)?;
...
pub fn pow(base: u128, exp: i32) -> Option<u128> {
    ...
    // The absolute value of i32::MIN cannot be represented as an i32, and attempting to calculate
