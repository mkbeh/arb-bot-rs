...
use crate::libs::solana_client::protocols::meteora_damm_v2::{
    CollectFeeMode, PoolFeesStruct,
    base_fee::BaseFeeHandler,
    constants::*,
    error::PoolError,
    fee::*,
    math::{safe_math::SafeMath, u128x128_math::Rounding, utils_math::*},
    params::{TradeDirection, fee_parameters::*},
...
impl PodAlignedFeeRateLimiter {
    ...
    pub fn get_max_index(&self) -> anyhow::Result<u64> {
    ...
    pub fn get_fee_numerator_from_included_fee_amount(
        &self,
        input_amount: u64,
    ) -> anyhow::Result<u64> {
    ...
    pub fn get_checked_amounts(&self) -> anyhow::Result<(u64, u64, bool)> {
    ...
    pub fn get_excluded_fee_amount(&self, included_fee_amount: u64) -> anyhow::Result<u64> {
    ...
    pub fn get_fee_numerator_from_excluded_fee_amount(
        ...
        let included_fee_amount = if excluded_fee_amount < checked_excluded_fee_amount {
            ...
            // ex * d * 2 * x0 = input_amount * d * 2 * x0 - input_amount * (2 * c * x0 + i *
            // (input_amount-x0)) ex * d * 2 * x0 = input_amount * d * 2 * x0 -
            // input_amount * (2 * c * x0 + i * input_amount- i*x0) ex * d * 2 * x0 =
            // input_amount * d * 2 * x0 - input_amount * 2 * c * x0 - i * input_amount ^ 2 +
            // input_amount * i*x0 i * input_amount ^ 2 - input_amount * (-2 * c * x0 +
            // i*x0 + d * 2 * x0) + ex * d * 2 * x0 = 0 equation: x * input_amount ^ 2
            // - y * input_amount + z = 0 x = i, y =  (-2 * c * x0 + i*x0 + d * 2 * x0),
            // z = ex * d * 2 * x0 input_amount = (y +(-) sqrt(y^2 - 4xz)) / 2x
            let i = U256::from(to_numerator(
                self.fee_increment_bps.into(),
                FEE_DENOMINATOR.into(),
            )?);
            ...
            let included_fee_amount =
                (y - sqrt_u256(y * y - four * x * z).ok_or(PoolError::MathOverflow)?) / (two * x);
        ...
        // sanity check
        if fee_numerator < self.cliff_fee_numerator {
...
impl BaseFeeHandler for PodAlignedFeeRateLimiter {
    fn validate(
        ...
        if !self.is_non_zero_rate_limiter() {
            return Err(PoolError::InvalidFeeRateLimiter.into());
        }
        ...
        if self.max_limiter_duration > max_limiter_duration {
            return Err(PoolError::InvalidFeeRateLimiter.into());
        }

        let fee_increment_numerator =
            to_numerator(self.fee_increment_bps.into(), FEE_DENOMINATOR.into())?;
        if fee_increment_numerator >= FEE_DENOMINATOR {
            return Err(PoolError::InvalidFeeRateLimiter.into());
        }

        let max_fee_bps_u64 =
            u64::try_from(self.max_fee_bps).map_err(|_| PoolError::TypeCastFailed)?;
        if max_fee_bps_u64 > get_max_fee_bps(CURRENT_POOL_VERSION)? {
            return Err(PoolError::InvalidFeeRateLimiter.into());
        }
    ...
    fn get_max_fee_numerator(&self) -> anyhow::Result<u64> {
        self.get_fee_numerator_from_included_fee_amount(u64::MAX)
    }
}
